...
while(<IN>){
	s/[\r\n]+$//g;
	@F=split/\t/;
	## 2022-10-18, sometimes a few lines do not have the fourth column (weird) ?!
	if(@F <= 3){
		print STDERR "Warning: Line '" . join("\t", @F) ."' does not have the fourth column, so I skip it\n";
		next;
	}
	@G=split(/;/, $F[3]);
	if(@G>0){
