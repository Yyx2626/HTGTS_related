...
if(!$should_skip){
	...
	my @required_fields = qw/Qname/;
	if($retrieve_element eq "junction"){
		push(@required_fields, qw/Rname Strand/);
		push(@required_fields, "Junction");
	}elsif($retrieve_element eq "prey"){
		push(@required_fields, qw/Rname Strand/);
		push(@required_fields, qw/Rstart Rend/);
	}elsif($retrieve_element eq "bait" or $retrieve_element eq "baitEnd"){
		push(@required_fields, qw/B_Rname B_Strand/);
		push(@required_fields, qw/B_Rstart B_Rend/);
	}else{
	...
	while(<IN>){
		...
		@F = split/\t/;
		$name = $F[$f2i{"Qname"}];
		if($retrieve_element eq "junction" || $retrieve_element eq "prey"){
			if($F[$f2i{"Rname"}] =~ /^\s*$/){
			...
			if($F[$f2i{"B_Rname"}] =~ /^\s*$/){
				print STDERR "Warning: no B_Rname info for line $rowidx, so I skip it\n";
				$rowidx++;
				next;
			}
			$chr = $F[$f2i{"B_Rname"}];
			$end = 0;
			$start = 0;
			if($F[$f2i{"B_Rstart"}] =~ /^\s*$/ || $F[$f2i{"B_Rend"}] =~ /^\s*$/){
				print STDERR "Warning: no B_Rstart or B_Rend info for line $rowidx, so I skip it\n";
				$rowidx++;
				next;
			}
			$end = $F[$f2i{"B_Rend"}];
			...
			if($retrieve_element eq "baitEnd"){
				if($strand eq "-"){
					$end = $start + 1;
				}else{
					$start = $end - 1;
